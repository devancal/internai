function ensureApplication(j){
  ...
  if(!a){a={id:crypto.randomUUID(),jobId:j.id,jobSnapshot:j,company:j.company,title:j.title,status:'Preparing',date:new Date().toISOString().slice(0,10),notes:'',draft:'',resumeNotes:'',answers:'',submittedAt:'',kitChoices:{}};state.apps.unshift(a);persist();}
  else{if(!a.jobSnapshot)a.jobSnapshot=j;if(!a.kitChoices)a.kitChoices={};persist()}
  return a;
}
...
function openPrep(appId){
  ...
  const resumeChoice=a.kitChoices.resume||'',coverChoice=a.kitChoices.cover||'',answersChoice=a.kitChoices.answers||'';
  ...
  <div class="panel">
    <h3>Your application materials</h3><p class="meta">Choose what you want help with. Your selections are saved and stay visibly selected.</p>
    <div class="kit-list">

      <div class="kit-item"><div class="kit-icon">📄</div><div><b>Tailored Resume</b><div class="meta">Your original resume is still usable. Tailoring can help the most relevant verified experience stand out for this role.</div><div class="choice-row" data-kit-choice-group="resume">${kitChoiceButton(a.id,'resume','tailored','Tailor my resume','resumeDetail',resumeChoice==='tailored')}${kitChoiceButton(a.id,'resume','original','Use original resume','resumeDetail',resumeChoice==='original')}</div></div><span class="kit-status recommended">Recommended</span></div>
      ...
      <div id="coverDetail" class="material-detail ${coverChoice==='generate'?'open':''}" style="grid-column:1/-1">
      ...
      <div id="answersDetail" class="material-detail ${answersChoice==='prepare'?'open':''}" style="grid-column:1/-1">
