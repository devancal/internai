...
function refreshSummaryAssist(){
  const hint=document.getElementById('summaryAssistHint'),input=document.getElementById('psummary');
  if(!hint||!input)return;
  if(!state.profile.resumeText){hint.innerHTML='Upload a resume below, then come back here to generate a summary.';return;}
  if(input.value.trim()){hint.innerHTML='You can keep typing your own summary. Clear the box to use the resume suggestion.';return;}
  hint.innerHTML='<b>✦ Resume suggestion ready.</b> Press <kbd style="padding:2px 6px;border:1px solid #cbd8ce;border-radius:5px;background:white;font-size:11px">Tab</kbd> to fill this from your resume, or just type your own.';
}
function useResumeSummaryAssist(){
  const input=document.getElementById('psummary');if(!input)return false;
