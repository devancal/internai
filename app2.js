...
function jobs(){
  ...
  <div class="panel" style="margin-top:16px"><div class="notice" style="margin-bottom:14px"><b>Using your profile by default:</b> ${fieldDefault?esc(fieldDefault):'Field not set'} · ${locationDefault?esc(locationDefault):'Location not set'} · ${esc(workDefault)}. These guide ranking automatically; edit the boxes below only when you want to override them for this search.</div><div class="form-grid"><div class="field"><label>Field / role <span class="meta">(from profile)</span></label><input value="${esc(fieldValue)}" placeholder="Mechanical Engineering" onfocus="if(!jobFilters.query&&this.value===${JSON.stringify(fieldDefault)})this.select()" oninput="jobFilters.query=this.value" onkeydown="if(event.key==='Enter')jobs()"><div class="meta" style="margin-top:6px">Your profile major is used automatically for ranking.</div></div><div class="field"><label>Location <span class="meta">(from profile)</span></label><input value="${esc(locationValue)}" placeholder="Cleveland, OH" onfocus="if(!jobFilters.location&&this.value===${JSON.stringify(locationDefault)})this.select()" oninput="jobFilters.location=this.value" onkeydown="if(event.key==='Enter')jobs()"><div class="meta" style="margin-top:6px">Your saved location and work preference are already part of the match score.</div></div><div class="field"><label>Work mode override</label><select onchange="setJobFilter('mode',this.value)"><option value="all" ${jobFilters.mode==='all'?'selected':''}>Use profile preference</option><option ${jobFilters.mode==='On-site'?'selected':''}>On-site</option><option ${jobFilters.mode==='Hybrid'?'selected':''}>Hybrid</option><option ${jobFilters.mode==='Remote'?'selected':''}>Remote</option></select></div><div class="field"><label>Saved</label><select onchange="setJobFilter('saved',this.value==='saved')"><option value="all" ${!jobFilters.saved?'selected':''}>All roles</option><option value="saved" ${jobFilters.saved?'selected':''}>Saved only</option></select></div></div><div class="row" style="margin-top:12px"><button class="btn dark small" onclick="jobs()">Apply overrides</button><button class="btn outline small" onclick="clearJobFilters()">Reset to profile</button><span class="meta">${ranked.length} result${ranked.length===1?'':'s'}</span></div></div>
  <div class="jobs" style="margin-top:16px">${ranked.length?ranked.map(jobHtml).join(''):'<div class="panel empty">No roles match those overrides.</div>'}</div>`;
}
