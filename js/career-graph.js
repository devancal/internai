...
relevantCareerEvidence=function(j){return graphEvidenceForJob(j).map(g=>({x:{id:g.node.id,type:g.node.type,title:g.node.title,source:g.node.source,skills:g.node.skills,claim:g.claimMatches[0]?.claim.text||''},score:g.score,hits:g.skillHits,tokenHits:g.claimMatches.flatMap(c=>c.tokenHits).slice(0,8)}))};
careerEvidenceCoverage=function(j){const map=graphRequirementMap(j);return map.length?map.filter(x=>x.supported).length/map.length:0};
function careerGraphStrength(j){const matches=graphEvidenceForJob(j);if(!matches.length)return 0;const strong=matches.filter(x=>x.claimMatches.length&&x.score>=8).length,distinctTypes=new Set(matches.map(x=>x.node.type)).size;return Math.min(1,strong*.25+distinctTypes*.15)}
const _opportunityPriorityGraph=opportunityPriority;opportunityPriority=function(j){return _opportunityPriorityGraph(j)+Math.round(careerGraphStrength(j)*6)};
function rebuildCareerGraph(){if(!state.profile?.resumeText)return;state.profile.careerGraph=buildCareerGraph(state.profile.resumeText);persist()}
const _parseResumeFileGraph=parseResumeFile;parseResumeFile=async function(file){await _parseResumeFileGraph(file);if(state.profile?.resumeText)rebuildCareerGraph()};
ensureCareerGraph();
